...
class Teacher:
    ...
    def train(self, train_index, train_loaders, test_loaders, test_train, test_val):
        ...
        # print(f"true_labels2:{true_labels2}")
    ...
    def aggregate_probability(self, model_votes, batch_size):
        """Aggregate model output into a single tensor of votes of all models.
           Args:
                votes: Model output
                n_dataset: Number of datapoints
        ...
        for model in model_votes:
            index = 0
            for val in model_votes[model]:
                # print(f'val[0]:{val[0]}; val[1]:{val[1]}')
                i = 0 if val[0] > val[1] else 1
                counts[index][0] += val[0]
        ...
        index = 0
        for val in model_votes[0]:
            counts[index][0] = counts[index][0]
            counts[index][1] = counts[index][1]

            # print(f'count[{index}]:[{counts[index][0]},counts[index][1]]:[{counts[index][1]}')
            index += 1
    ...
    def predict(self, data, mask, outputs):
        ...
        with torch.no_grad():
            for i in range(self.n_teachers):
                ...
                model_probability[i]=softmax(outputs[i], dim=1).cpu().numpy()
        ...
        # print(f'2:probability:{probability}')
        probability= softmax(probability, dim=1).cpu().numpy()
        # print(f'probability_distribution:{probability}')
        ...
        output = {"predictions": predictions, "predictions_nonoise": predictions_nonoise, "counts": counts,
                  "model_counts": model_counts, "probability_distribution": probability}
